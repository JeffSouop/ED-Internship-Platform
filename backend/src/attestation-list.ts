...
import { listAttestationIndexEntries } from "./attestation-index.js";
...
export async function listAttestations(pool: pg.Pool): Promise<AttestationListRow[]> {
  const entries = listAttestationIndexEntries();
  if (entries.length === 0) return [];

  const studentIds = entries.map((e) => e.studentId);
  const summaries = await loadStudentSummaries(pool, studentIds);

  const rows: AttestationListRow[] = entries.map(({ studentId, entry }) => {
