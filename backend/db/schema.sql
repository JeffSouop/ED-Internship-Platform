...
-- ---------------------------------------------------------------------
-- Journal brut : chaque envoi de formulaire convention (audit indépendant
-- du statut de validation — validé, refusé ou en attente).
-- ---------------------------------------------------------------------
CREATE TABLE internship_form_response (
    id               UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    student_id_text  TEXT NOT NULL,              -- numéro étudiant (Symplicity) tel que saisi
    payload          JSONB NOT NULL,             -- corps JSON reçu (copie intégrale)
    submission_id    UUID REFERENCES student_submission(id) ON DELETE SET NULL,
    created_at       TIMESTAMPTZ NOT NULL DEFAULT now()
);

CREATE INDEX idx_form_response_student ON internship_form_response(student_id_text);
CREATE INDEX idx_form_response_created ON internship_form_response(created_at DESC);
CREATE INDEX idx_form_response_sub ON internship_form_response(submission_id);
...
CREATE TABLE company_declaration (
    ...
    partner_form_extras JSONB,                   -- section 5 & métadonnées formulaire partenaire
    submitted_at  TIMESTAMPTZ NOT NULL DEFAULT now(),
    updated_at    TIMESTAMPTZ NOT NULL DEFAULT now(),
...
CREATE TABLE declared_intern (
    ...
    first_name      TEXT,
    last_name       TEXT,
    internship_type TEXT,                      -- ex. culinary | pastry
    position        TEXT NOT NULL,
    start_date      DATE NOT NULL,
    end_date        DATE NOT NULL,
    tutor_name      TEXT,
...
-- Snapshot formulaire partenaire (grille métier) : une ligne par stagiaire déclaré au moment de l’envoi.
