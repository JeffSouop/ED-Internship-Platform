-- Merge uniquement sur student_id (plus de contrainte par rentrée).
SET search_path TO careers, public;

DROP VIEW IF EXISTS careers.v_merged_overview CASCADE;

-- Une seule ligne par student_id : supprimer les doublons (garder merged_at le plus récent).
DELETE FROM careers.merged_internship mi
 WHERE mi.id IN (
         SELECT id FROM (
             SELECT id,
                    ROW_NUMBER() OVER (
                        PARTITION BY student_id ORDER BY merged_at DESC NULLS LAST, id DESC
                    ) AS rn
               FROM careers.merged_internship
         ) d
        WHERE d.rn > 1
       );

ALTER TABLE careers.merged_internship
  DROP CONSTRAINT IF EXISTS merged_internship_student_id_season_year_key;

ALTER TABLE careers.merged_internship
  DROP CONSTRAINT IF EXISTS merged_internship_student_id_key;

ALTER TABLE careers.merged_internship
  ADD CONSTRAINT merged_internship_student_id_key UNIQUE (student_id);

DROP FUNCTION IF EXISTS careers.try_merge_internship(text, careers.intake_season, integer);
DROP FUNCTION IF EXISTS careers.try_merge_internship(text, intake_season, integer);
...
CREATE OR REPLACE FUNCTION careers.trg_submission_after_review()
RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
    IF NEW.status = 'approved'
       AND (TG_OP = 'INSERT' OR OLD.status IS DISTINCT FROM NEW.status) THEN
        PERFORM careers.try_merge_internship(NEW.student_id);
    END IF;
    RETURN NEW;
END $$;

CREATE OR REPLACE FUNCTION careers.trg_declared_intern_after_change()
RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
    PERFORM careers.try_merge_internship(NEW.student_id);
    RETURN NEW;
...
BEGIN
  FOR sid IN
      SELECT DISTINCT student_id FROM (
        SELECT student_id FROM careers.student_submission WHERE status = 'approved'
        UNION
        SELECT student_id FROM careers.declared_intern
      ) q
  LOOP
    PERFORM careers.try_merge_internship(sid);
  END LOOP;
END $$;
