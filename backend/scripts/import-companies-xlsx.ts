/**
 * Importe les entreprises historiques depuis « COMPANY DATABASE.xlsx » (racine du projet).
 *
 * Usage :
 *   cd backend
 *   npm run import:companies
 *   npm run import:companies -- ../COMPANY\ DATABASE.xlsx
 *
 * Prérequis : DATABASE_URL dans backend/.env, Postgres démarré.
 */
...
try {
  ...
  console.log(`Lignes Excel : ${allRows.length} → ${rows.length} entreprises uniques (nom + pays)`);
  ...
  console.log(`  Nouvelles fiches : ${inserted}`);
  console.log(`  Fiches mises à jour : ${updated}`);
  console.log(`  Contacts RH / tuteur : ${contacts}`);
  console.log(`  Entreprises sans contact valide : ${skippedContacts}`);
  console.log(`  Total en base : ${countRows[0]?.n ?? "?"}`);
} catch (e) {
  console.error(e);
  process.exit(1);
} finally {
